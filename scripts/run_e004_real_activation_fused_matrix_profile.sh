...
PROFILE_STEM="$ROOT/.local/profiles/e004-real-activation-fused-matrix"
RUN_EVIDENCE="$ROOT/artifacts/E004-qwen3-layer-capture/real-activation-fused-matrix/fused-matrix.json"
ARTIFACT_ROOT="$ROOT/artifacts/E004-qwen3-layer-capture/real-activation-fused-matrix"
...
PYTHONPATH="$ROOT/src" nsys profile \
  ...
  -m scripts.run_e004_real_activation_fused_matrix \
  --model-dir "$MODEL_DIR" \
...
PYTHONPATH="$ROOT/src" "$LAB_ROOT/.venv/bin/python" \
  -m scripts.finalize_e004_real_activation_fused_matrix \
